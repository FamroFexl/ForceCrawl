...
public class ConfigScreen extends Screen {
	...
	public enum CrawlType {
		...
		CrawlType(String name) {
			this.name = (Component) MutableComponent.create((ComponentContents) new TranslatableContents(name));
		}
	...
	protected ConfigScreen(Screen parent) {
		super(MutableComponent.create((ComponentContents) new TranslatableContents("config.forcecrawl.title")));
	}
	...
	public void render(PoseStack poseStack, int i, int j, float f) {
		...
		GuiComponent.drawString(poseStack, this.font, FormattedCharSequence.forward(MutableComponent.create((ComponentContents) new TranslatableContents("config.forcecrawl.crawlMode")).getString(), Style.EMPTY.withColor(ChatFormatting.GRAY)), this.width / 2 - 180, 50, 0xFFFFFF);
		//Render widgets
